...
/* Thread Replies(POST, PUT, DELETE) */
...
app.post(/^\/forum\/thread\/like/, function(req, res) {
    ...
    Thread.findOne({ _id : body.thread_id }, (err, thread) => {
        if(err) {
            simpleResponseQuery(res, 401, "couldn't like the thread" + err.toString());
        } else {
            thread.likes.set(req.user._id);
            thread.save();
...
app.delete(/^\/forum\/thread\/like/, function(req, res) {
    ...
    Thread.findOne({ _id : body.thread_id }, (err, thread) => {
        if(err) {
            ...
            thread.likes.delete(req.user._id);
            thread.save();
        }
    });
});
...
app.post(/^\/forum\/thread\/reply\/like/, function(req, res) {
    ...
    Thread.findOne({ '_id' : body.thread_id, 'replies.id' : body.reply_id }, (err, reply) => {
        if(err) {
            simpleResponseQuery(res, 401, "couldn't like thread's reply" + err.toString());
        } else {
            reply.likes.set(req.user._id);
            reply.save();

            simpleResponseQuery(res, 200, reply, "application/json");
        }
    });
});
...
app.delete(/^\/forum\/thread\/reply\/like/, function(req, res) {
    ...
    Thread.findOne({ '_id' : body.thread_id, 'replies.id' : body.reply_id }, (err, reply) => {
        if(err) {
            simpleResponseQuery(res, 401, "couldn't delete the like of thread's reply" + err.toString());
        } else {
            reply.likes.delete(req.user._id);
            reply.save();
...
app.put(/^\/forum\/thread\/reply/, function(req, res) {
    ...
    Thread.update({ _id : thread_id, "replies.author" : req.user._id },
...
app.delete(/^\/forum\/thread\/reply/, function(req, res) {
    ...
    Thread.update({ _id : body.threadID, "replies.author" : req.user._id }, {
        $pull : {
